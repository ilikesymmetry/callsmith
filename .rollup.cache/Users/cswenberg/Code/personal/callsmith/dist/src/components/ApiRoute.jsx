...
export function ApiRoute({ state, dispatch, }) {
    const route = state.routes[state.selectedRoute];
    const onBodyChange = useCallback((v) => {
        ...
        try {
            const path = route.path
                ...
                : route.path
                    ...
                    .join("/");
            const query = route.query.filter((kv) => kv.key !== "").length === 0
                ? ""
      ...
      <ResizablePanel defaultSize={50} minSize={8} className="px-8 py-4">
        ...
        <div className="flex justify-between items-center">
          <div className="space-x-2 flex items-center">
            ...
            <div className="text-lg flex items-center">
              {pathItems.map((name, i) => (<ApiRoutePathNode key={`${name}-${i}`} name={name}/>))}
            </div>
          </div>
        ...
        <Tabs className="mt-4 h-full" defaultValue="headers">
          <TabsList>
            ...
            ? ""
            : "hidden"} alert={!route.params.every((kv) => !!kv.value)}>
              Params
            </TabsTrigger>
          </TabsList>
