...
export function KeyValueForm({ keyValues, setKeyValues, lockKeys = false, }) {
    ...
    return (<Table className="space-y-2 w-2/3">
      ...
      <TableBody>
        {keyValues.map((keyValue, i) => (<TableRow className="" key={i}>
            <TableCell>
              <Input name={`fields.${i}.key`} value={keyValue.key} placeholder="Key" onChange={(e) => {
                handleChange(e);
            }} disabled={lockKeys}/>
            </TableCell>
            <TableCell>
              <div className="relative">
                <Input name={`fields.${i}.value`} value={keyValue.value} placeholder="Value" onChange={(e) => {
                handleChange(e);
            }}/>
                <button className="absolute top-2 -right-8" hidden={i === keyValues.length - 1} onClick={() => {
